...
//stripe + client test secret key (Could not get this to work as an environment variable)
const stripe = require("stripe")(
  "sk_test_51R6n86KbzkDbosBfleOqPMx3aUTzQMldA8IqvuSOBKkhb0s0f0CV8axkLXoyYFyVlpl5JmupjyBi0Uq82pLK7a3q00hlx18kvk"
);
require("dotenv");

//Creates transporter object to send emails
const transporter = nodemailer.createTransport({
  //select email service and authenticate account
  service: "gmail",
  auth: {
...
//METHOD: sends email containing order information to user
async function sendOrderConfirmation(customerOrder, customerEmail) {
  // Generate the list items HTML for the order items
  const itemsListHtml = customerOrder.orderItems.products
    .map(
      (item) =>
        `<li>
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Shipping Address: ${
          customerOrder.shippingAddress || "Not provided"
        }</p>
...
//METHOD: Creates/Configures embedded stripe form
router.post("/create-checkout-session", protectRoutes, async (req, res) => {
  try {
    ...
    if (
      ...
      user.userCart.products.length === 0
    ) {
      return res
        .status(404)
...
//METHOD: Creates Order and sends confirmation order once transaction goes through
router.get("/session-status", async (req, res) => {
  try {
      ...
      try {
        ...
        if (user && user.userCart) {
          ...
          const newOrder = {
            ...
            shippingAddress:
              session.shipping_details?.address?.line1 || "Not provided",
    ...
    //send session status, email, and orderdata
    res.send({
